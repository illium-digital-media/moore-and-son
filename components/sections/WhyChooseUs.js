...
const iconStyle = 'w-auto mb-6 block duration-300';
...
const WhyChooseUs = () => {
  return (
    <section className={`p-section text-center ${styles.section}`}>
      <div className={`w-full max-w-xl m-auto flex flex-col items-center gap-12 md:gap-16 lg:gap-20 md:max-w-7xl ${styles.container}`}>
        <div className={`text-left w-full flex flex-col items-start md:flex-row md:justify-between md:items-end md:gap-16 ${styles.content}`}>
          <div className={`md:w-1/2`}>
            <Topper text="Why Choose Us" />
            <Title title='Expert Painting & Decorating Service You Can Trust' color="black" />
          </div>
          <p className={`max-w-lg ${styles.text}`}>
        ...
        <ul className={`w-full flex justify-center items-center flex-col gap-4 ${styles.cardGroup}`}>
          {items.map((item, index) => (
            <li className={`text-left w-full p-8 bg-gray-100 rounded-xl duration-300 border-box hover:bg-primary cursor-pointer ${styles.item}`} key={index}>
              {item.icon}
